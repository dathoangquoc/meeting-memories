...
export function SingleNote({ note, onDelete, onSave }: SingleNoteProps) {
  return (
    <Card className="group mx-auto w-full min-w-36 min-h-72 max-w-sm max-h-80 shadow-md">
      <CardHeader className="flex flex-row justify-between border-b-2 pb-2 h-fit">
        <CardTitle className="flex flex-row my-2">
          {note.title ?? "Empty"}
        </CardTitle>
        <div className="h-fit opacity-0 group-hover:opacity-100 transition-all duration-200 z-50 mt-0">
          <UpdateNoteDialog
            noteData={note}
            onSubmit={onSave}
            onDelete={onDelete}
